...
### 7.1. Эволюция Аватара (From Advisor to Trustee)
...
3.  **Уровень 3: Активный Защитник.**
    *   Имеет право "Вето" (блокировка кнопки "Купить" при высоком риске).
    *   Требует текстового обоснования нерациональных сделок.
...
### 7.2. Принципы UI/UX (Telegram)

1.  **Единая Карточка События (Single Source of Truth UI)**
    *   Вместо потока разрозненных сообщений, бот присылает **одну карточку** (Asset Card), которая обновляется или дополняется.
    *   В карточке есть секция Брокера (Call to Action) и секция Аватара (Risk Warning).

2.  **Разделение Голосов**
    *   Брокер всегда говорит первым при событии Рынка (он быстрее).
    *   Аватар говорит первым при событии Действия Пользователя (он перехватывает руку).
