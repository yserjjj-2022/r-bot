...
### 4.4 Эмпатическое Прогнозирование (Empathy & Prediction Error)
Превращение "галлюцинаций LLM" (когда бот пишет реплики за пользователя) в механизм социального интеллекта.
*   **Проблема:** LLM иногда генерирует `Human: ...`, предсказывая реакцию пользователя.
*   **Решение:**
    1.  **Sanitization:** Мы отрезаем этот текст из финального ответа пользователю (скрываем галлюцинацию).
    2.  **Internal Use:** Мы сохраняем этот "прогноз" как ожидание (Expectation).
    3.  **Feedback Loop:** На следующем шаге сравниваем *реальный ответ* пользователя с *прогнозом*.
        *   Совпадение -> Рост уверенности и эмпатии (Dopamine).
        *   Ошибка -> Рост Arousal (Surprise) и переоценка модели собеседника.
