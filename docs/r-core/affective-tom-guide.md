...
- ✅ **Phase 2.1 (Done)**: Affective Extraction, VAD Mapping, Basic Retrieval.
- ⏳ **Phase 2.2 (In Progress)**:
  - Integration with **Predictive Processing** (EA влияет на точность ToM).
  - **Vector Traits**: Поиск похожих профилей через эмбеддинги (`user_similarity`).
- ⏳ **Phase 2.3 (Planned)**:
  - **Temporal Decay**: Реализация формулы Эббингауза для эмоций.
  - **Shadow Profile UI**: Дашборд для просмотра всех черт (см. Phase 2.5).
- ⏳ **Phase 3 (Future)**:
  - **Strategic Deception**: Способность скрывать знания ToM, если это этически оправдано.
