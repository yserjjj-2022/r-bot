...
# Финальная версия: ИИ теперь помнит последний диалог с пользователем
...
def build_full_context_for_ai(db: Session, session_id: int, user_id: int, current_question: str, options: list) -> str:
    """Собирает полный контекст, включая финансовое состояние и ПОСЛЕДНИЙ ДИАЛОГ С ИИ, для системного промпта."""
    ...
    # 3. *** НОВАЯ ЛОГИКА ***
    # Получаем ПОСЛЕДНИЙ состоявшийся диалог с ИИ в этой сессии
    last_ai_dialogue = db.query(models.AIDialogue).filter(models.AIDialogue.session_id == session_id).order_by(models.AIDialogue.id.desc()).first()
    
    ai_history_context = ""
    if last_ai_dialogue:
        ai_history_context = (
            "**СТЕНОГРАММА ПРЕДЫДУЩЕГО ДИАЛОГА (для контекста):**\n"
            f"Пользователь спросил: «{last_ai_dialogue.user_message}»\n"
            f"Ты ответил: «{last_ai_dialogue.ai_response}»\n\n"
        )
    ...
    system_prompt = (
        "Ты — финансовый консультант в игре. Твоя задача — проанализировать всю информацию и дать пользователю развернутый, аргументированный совет.\n\n"
        "--- ДАННЫЕ ДЛЯ АНАЛИЗА ---\n"
        f"{ai_history_context}" # Вставляем стенограмму последнего диалога
        f"**Текущая игровая ситуация:**\n{current_question}\n\n"
        f"**Предложенные варианты выбора:**\n{options_text}\n\n"
        f"**Финансовая сводка по игроку:**\n{state_info}\n\n"
        f"**История предыдущих решений игрока:**\n{history}\n\n"
        "--- ЗАДАЧА ---\n"
        "Основываясь на ВСЕХ этих данных, ответь на следующий вопрос пользователя. Не задавай уточняющих вопросов о том, что уже было сказано. Будь краток и давай конкретную рекомендацию."
    )
