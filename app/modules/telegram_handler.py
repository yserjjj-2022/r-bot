...
# Финальная версия 5.15: Улучшенная логика завершения игры
...
def register_handlers(bot: telebot.TeleBot, graph_data: dict):
    ...
    def send_node_message(chat_id, node_id):
        ...
        try:
            ...
            if not session_info:
                # Если сессии нет, но мы пытаемся отправить сообщение, значит игра уже закончилась.
            ...
            # Обработка типов узлов, которые не отправляют основное сообщение
            if node_type == "pause":
            ...
            # --- ИЗМЕНЕНИЕ: Проверка на финальный узел и завершение сессии ---
            ...
            # Для узла "Состояние" автоматически переходим к следующему узлу
            if node_type == "state":
                next_node_id = node.get("next_node_id")
                if next_node_id:
                    send_node_message(chat_id, next_node_id)
    ...
    def handle_callback_query(call):
        ...
        try:
            ...
            if node.get("type") == "circumstance":
            ...
            elif node and "options" in node and len(node["options"]) > button_idx:
                ...
                if node.get("type") == "task" and "formula" in option_data:
